...
@PreventIframe("/testMtzaMessageExtension/config.html")
@PreventIframe("/testMtzaMessageExtension/action.html")
export default class TestMtzaMessageExtension implements IMessagingExtensionMiddlewareProcessor {
    ...
    public async onFetchTask(context: TurnContext, value: IMessagingExtensionActionRequest): Promise<MessagingExtensionResult | ITaskModuleResult> {
        ...
        return Promise.resolve<ITaskModuleResult>({
            ...
            value: {
                title: "Input form",
                url: `https://${process.env.HOSTNAME}/testMtzaMessageExtension/action.html`
            }
        });
